1) Nel codice sorgente della directory principale c'è un messaggio indirizzato probabilmente ai developer che però ci dice un possibile nickname da usare per fare l'accesso a ssh:
<!-- john, please add some actual content to the site! lorem ipsum is horrible to look at. -->

2) Nella directory /robots.txt possiamo vedere una directory interessante a cui può accedere qualsiasi user-agent: 
user-agent: *
Allow: /
/uploads/ 
3) Nella directory /uploads/ troviamo diversi file:

dict.lst <--- è una lista per attacco bruteforce
manifesto.txt 
meme.jpg <--- è un immagine

4) Vediamo il contenuto di manifesto.txt:
...
5)  Il manifesto è interessante ma non ci dice niente di interessante per risolvere la CTF
6) Se non ci fosse la wordlist tra i file analizzerei l'immagine cercando dei metadati o dei file nascosti nell'immagine ma secondo me non ci dice niente al massimo la analizzo in un secondo momento
7) L'unica opzione è quella di copiarsi la wordlist in locale e usarla per bruteforzare il servizio ssh conoscendo un nickname ovvero john