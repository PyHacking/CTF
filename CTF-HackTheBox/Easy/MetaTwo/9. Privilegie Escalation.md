1) Ci st nella directory una cartella ".passpie": cd passpie
2) Vediamo cosa contiene questa directory: ls -all
total 24
...
4) Vediamo cosa contiene il file .keys: cat .keys
...
5) Copiamo la Chiave privata in locale con nano: nano id_rsa
6) La portiamo in un file che può leggere john the ripper:
...
9) Dentro questo file andiamo a sovrascriverlo con le password che stanno nella directory  /home/jnelson/.passpie/ssh, però le decriptiamo usando la Passphrase scoperta con john the ripper: passpie export pass

10) Andiamo a vedere le password racchiuse nel file "pass": cat pass
credentials:
- comment: ''
  fullname: root@ssh
  login: root
...
11) Abbiamo scoperto la password root: 
